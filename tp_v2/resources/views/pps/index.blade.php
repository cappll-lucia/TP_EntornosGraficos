...
@section('content')
                                    ...
                                    @foreach ($pps as $app)
                                        <tr data-id="{{ $app->id }}" class="clickable" data-url="/pps/details">
                                            ...
                                            @if ($app->Responsible == null)
                                                <td>-</td>
                                            @else
                                                <td>{{ $app->Responsible->last_name }},
                                                    {{ $app->Responsible->first_name }}</td>
                                            @endif
    ...
    <script>
        $('#DataTable').DataTable({
            ...
            "columnDefs": [{
                "orderable": false,
                "targets": 0 // Esto hace que la primera columna no sea ordenable
            }],
            "order": [
                [1, 'desc']
            ] // Ordenar por la columna 'Nro' (columna 1) de mayor a menor
        });
